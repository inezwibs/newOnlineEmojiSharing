...
async function getStudentClassId(req, res, next) {
    ...
    try {
        ...
        var resultArr = rows[0].datetime.split(/[,-]/);
        ...
        if (resultDateArr.length > 0 ){
            ...
            await invalidEmojiPostBranch(req,res);
        }
    } catch (e) {
        console.log("Catch an error: ", e);
    }
}
async function invalidEmojiPostBranch(req,res) {
...
async function checkRecordExistsInPostedEmojis(req, res, next) {
    //get currenMinutes and req.classstartminutes from getclassstarttime
    if (req.currentMinutes && req.classStartMinutes){
        req.insertMinutes = req.currentMinutes - req.classStartMinutes;
    }
    let recordExistsInPostedEmojis  = false;
    let query =
...
async function insertEmojiRecord(req, res, next) {
    ...
    // if record exists already we update it again for this minute for this user
    let query;
...
//         req.user + " and class_id = '" + req.class_id + "' and " + " minute = " + thisMinute
//     try {
//         const [rows, err] = await db.execute(query);
//         if (rows.length > 0) {
...
async function checkRecordExists(req, res, next) {
    ...
    req.thisUser = req.user ? req.user : req.body.userId;
    let recordExists = false;
    if (req.thisMinute && req.thisUser && req.class_id){
        query =
...
async function getContributedStudentsCount(req, res, next) {
    let query =
        " SELECT count(distinct users_id) as count FROM emojidatabase.posted_emojis where class_id = " +
        req.class_id + " and minute = " + req.thisMinute;

    try {
        const [rows, fields] = await db.execute(query);
        if (rows != null){
            req.contributedStudents = rows[0].count;
        } else {
            req.contributedStudents = 0;
        }
        next();
    } catch (e) {
        console.log("Catch an error: ", e);
...
async function insertRecordPerMinute(req, res, next) {
    ...
    let query;
    req.studentNotContributed = req.classRegisteredStudentsCount - req.contributedStudents;
...
async function getEmojiClassData(userInfo, classLinkId, classId) {
    ...
    if (userInfo.length > 4) {
        ...
        userQuery = "SELECT u.full_name, u.id,  c.class_name, c.datetime, r.classes_id " +
            "FROM emojidatabase.users u, emojidatabase.registrations r, emojidatabase.classes c " +
            "WHERE u.id = r.users_id AND c.id = r.classes_id AND u.id = " + userInfo ;
    }
    let result;
    try {
        ...
        if (rows === undefined || rows.length === 0) {
            ...
            result = 0;
        } else if (classId != null ) {// cathinh null and undefined
            rows.forEach( row => {
        ...
        } else if (classId === 0 || classId == null && rows){
